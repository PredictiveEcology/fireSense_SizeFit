...
#    `functionName` is a function's name in your module (e.g., `fireSense_SizeFitEvent1`).
# 2. Copy this file to the tests folder (i.e., ` # replace with empty string insteadfireSense_SizeFit/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSense_SizeFit")
  ...
  parameters <- list(
    ...
    fireSense_SizeFit = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSense_SizeFitEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSense_SizeFitEvent1(mySim)
  } else {
    simOutput <- mySim$fireSense_SizeFitEvent1(mySim)
  }
  ...
  if (exists("fireSense_SizeFitEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSense_SizeFitEvent2(mySim)
  } else {
    simOutput <- mySim$fireSense_SizeFitEvent2(mySim)
  }
